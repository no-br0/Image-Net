...
def get_vram_usage(include_thermals=True):
...
def log_vram_usage(include_thermals=True):
    """Log VRAM/thermals using query_vram_usage; choose stdout and/or file logging."""
    stats = get_vram_usage(include_thermals=include_thermals)
    with open(GPU_LOG_PATH, "a", encoding="UTF8") as gpu_log:
        print(f"VRAM Used: {stats['vram_used']:.2f} MB / {stats['vram_total']:.2f} MB", file=gpu_log)
        print(f"CuPy Pool:\n", 
              f"    Used: {stats['pool_used']:.2f} MB\n",
              f"    Total: {stats['pool_total']:.2f} MB\n",
              f"    Free {stats['pool_free']:.2f} MB", file=gpu_log)
        if include_thermals:
            print(f"GPU Temp: {stats['gpu_temp']}°C | Utilization: {stats['gpu_util']}%", file=gpu_log)
        print("", file=gpu_log)
        gpu_log.flush()

    #line = (f"VRAM: {stats['vram_used']:.1f}/{stats['vram_total']:.1f} MB "
    #        f"| Pool: {stats['pool_used']:.1f}/{stats['pool_total']:.1f} MB "
    #        f"| Temp: {stats['gpu_temp']}°C | Util: {stats['gpu_util']}%")
    #print(line)
